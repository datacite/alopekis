...
def process_month_job(worker_id: int, job: dict, work_queue: JoinableQueue, results_queue: JoinableQueue, logger: logging.Logger) -> None:
    ...
    try:
        ...
        for result in results:
            ...
            if results_count % 10000 == 0:
                ...
                if (expected_count and expected_count >= 1000000 and results_count % 200000 == 0) or (expected_count and 100000 <= expected_count < 1000000 and results_count % 50000 == 0):
        ...
        results_queue.put({"year": year, "month": month, "count": results_count, "status": "final", "registered": registered_count, "findable": findable_count}, block=True)
...
def month_worker(worker_id: int, work_queue: JoinableQueue, results_queue: JoinableQueue, log_queue: Queue, max_retries: int = 3) -> None:
    ...
    while True:
        ...
        try:
            ...
            logger.error(f"Worker {worker_id} encountered a fatal error processing {job.get('year')}-{job.get('month')}, sending failure status to results queue")
            try:
                results_queue.put({"year": job.get('year'), "month": job.get('month'), "count": 0, "status": "failed"}, block=True)
            except Exception:
