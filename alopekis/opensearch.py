...
from .config import OPENSEARCH_HOST, OPENSEARCH_INDEX, OPENSEARCH_PORT
    ...
    def return_all_results(self) -> List[Hit]:
        ...
        while not query_finished:
            ...
            try:
                ...
                if (
                ...
                ):  # This is the tiemout from OpenSearch rather than the connection
                    ...
                    timeout_count += 1
                    self.logger.info(
                        f"Query timed out at OpenSearch level (count: {timeout_count}): {self.query.to_dict}"
                    )
                    if timeout_count > 10:
                        ...
                        self.logger.warning(
                            "Timeout, sleeping for 10 seconds and retrying"
                        )
                        time.sleep(10)
            except (Timeout, TimeoutError):
                timeout_count += 1
                self.logger.info(
                    f"Query timed out at network level (count: {timeout_count}): {self.query.to_dict}"
                )
                if timeout_count > 10:
                    self.logger.error("Too many timeouts, giving up")
                    raise TooManyTimeouts
